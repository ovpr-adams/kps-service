...
const Header = () => {
  ...
  const navItems = [
    ...
    { name: 'Zones', path: '/zones', section: 'zones' },
          ...
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center space-x-6">
              <div className="flex items-center space-x-2">
                <Phone size={14} />
                <span>{settings?.phone || '+33652323256'}</span>
              </div>
              <div className="flex items-center space-x-2">
                <Mail size={14} />
                <a href={`mailto:${settings?.publicEmails?.[0] || 'contact@kpsservices.fr'}`} className="hover:text-green-400 transition-colors">{settings?.publicEmails?.[0] || 'contact@kpsservices.fr'}</a>
              </div>
              <div className="flex items-center space-x-2">
                <MapPin size={14} />
                <span>{(settings?.serviceAreas || ['Île-de-France','Nantes','Rouen']).join(', ')}</span>
              </div>
            </div>
            <div className="text-green-400 font-medium">
              Horaires : {settings?.businessHoursText || '7h00-19h30'}
            </div>
          </div>
        </div>
      </div>
